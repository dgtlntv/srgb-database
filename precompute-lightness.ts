 ...
 */
function scaleToContrast(scaleValue: number): ContrastRatio {
 ...
 */
function reverseWCAGContrast(
    contrast: ContrastRatio = 4.5,
    y: Luminance = 1
): Luminance {
    if (!(y >= 0 && y <= 1)) {
        throw new Error(
            `Invalid luminance value: ${y} (must be between 0 and 1)`
        )
    }
    if (!(contrast >= 1 && contrast <= CONFIG.WCAG_MAX_CONTRAST)) {
        throw new Error(
            `Invalid contrast ratio: ${contrast} (must be between 1 and ${CONFIG.WCAG_MAX_CONTRAST})`
        )
    }
 ...
 */
function yToOklabLightness(inputY: Luminance): Lightness {
    ...
    const L =
        LMS_TO_OKLAB_MATRIX[0][0] * l_prime +
 ...
 */
function toe(L: Lightness): Lightness {
 ...
 */
function yToOkhslLightness(inputY: Luminance): Lightness {
 ...
 */
function computeLightness(step: number): Lightness {
 ...
 */
function computeAllLightnessValues(): Record<number, number> {
    const lightnessValues: Record<number, number> = {}

    for (let step = 0; step <= CONFIG.SCALE_MAX; step++) {
        const lightness = computeLightness(step)
        const roundedLightness =
            Math.round(lightness * CONFIG.LIGHTNESS_PRECISION) / CONFIG.LIGHTNESS_PRECISION
        lightnessValues[step] = roundedLightness

        if (step % CONFIG.PROGRESS_INTERVAL === 0) {
            console.log(
                `Step ${step}: lightness = ${lightness.toFixed(
                    6
                )} (rounded: ${roundedLightness})`
            )
        }
    }
